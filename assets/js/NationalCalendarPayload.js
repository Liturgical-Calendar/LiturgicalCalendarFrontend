...
class NationalCalendarPayloadMetadata {
     ...
     * @param {string} metadata.wider_region One of `Americas`, `Europe`, `Asia`, `Africa`, `Oceania`, or `Middle East`.
    ...
    constructor( metadata ) {
        ...
        const re2 = /^(Americas|Europe|Asia|Africa|Oceania|Middle East)$/;
        if (false === re2.test(metadata.wider_region)) {
            throw new Error('`metadata.wider_region` parameter must be one of `Americas`, `Europe`, `Asia`, `Africa`, `Oceania`, or `Middle East`');
        }
