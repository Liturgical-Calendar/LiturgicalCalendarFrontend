...
/**
 * Scenario 06 — rome-editor requests editor@diocesan_calendar:romamo_it
 *
 * Diocesan analogue of scenario 02 (national calendar editor grant).
 * The seeded rome-editor (who starts without the editor FGA tuple) requests
 * editor@romamo_it; the rome-admin (admin@romamo_it) approves it.
 *
 * Visibility matrix:
 ...
 *   - rome-editor:  Zitadel calendar_editor role, NO FGA tuple (earned via request)
...
test('06 — rome-editor edit@romamo_it: rome-admin approves; scoped visibility', async ({ browser }) => {
    ...
    // ── Step 1: rome-editor submits a request for editor@diocesan_calendar:romamo_it ──
    const romed = await actingAs(browser, 'rome-editor');
    try {
        await submitAccessRequest(romed.page, {
            requestedRole: 'calendar_editor',
            permission: { objectType: 'diocesan_calendar', objectId: 'romamo_it', relation: 'editor' },
        });
    } finally {
        await romed.context.close();
    }
    ...
    try {
        expect(
            await requestVisible(sa.page, { requesterEmail: USERS['rome-editor'].email }),
        ).toBe(true);
    } finally {
    ...
    try {
        expect(
            await requestVisible(romeadm.page, { requesterEmail: USERS['rome-editor'].email }),
        ).toBe(true);
    } finally {
    ...
    try {
        expect(
            await requestVisible(ceiadm.page, { requesterEmail: USERS['rome-editor'].email }),
        ).toBe(false);
    } finally {
    ...
    try {
        expect(
            await requestVisible(usccbadm.page, { requesterEmail: USERS['rome-editor'].email }),
        ).toBe(false);
    } finally {
        await usccbadm.context.close();
    }

    // ── Step 3: rome-admin approves the request ───────────────────────────────
    const romeadm2 = await actingAs(browser, 'rome-admin');
    try {
        await actOnRequest(romeadm2.page, {
            requesterEmail: USERS['rome-editor'].email,
            action: 'approve',
            notes: 'ok',
        });
    } finally {
        await romeadm2.context.close();
    }

    // ── Step 4: Assert FGA tuple editor@diocesan_calendar:romamo_it now exists ──
